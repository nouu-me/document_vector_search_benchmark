...
class MIRACL(Dataset):
    def __init__(
        ...
        self.split = split
        self.name = f"MIRACL-v{version}-{split}"
    ...
    def load_data(
        ...
        split: str,
        cache: bool,
        sampling_method: Optional[Literal["hard_negative", "random"]],
        corpus_sample: int,
    ) -> None:
        if cache:
            cache_dir = self.get_cache_dir()
            if cache_dir.exists():
                self.__load_cache()
                return
        logger.info(f"loading MIRACL dataset (version: {version}, split: {split})")
        self.titles = []  # Unused
        self.queries = []
        self.contexts = []
        self.related_context_locations = []
        data = datasets.load_dataset("miracl/miracl", "ja", split)[split]
        context_to_index = {}  # Map context text to its index

        if sampling_method is not None:
            corpus = datasets.load_dataset("miracl/miracl-corpus", "ja")["train"]
